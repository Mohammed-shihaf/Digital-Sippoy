...
console.log("Syncing .gitignore across all 32 branches...");
...
for (const branch of ALL_TARGETS) {
  ...
  if (diff && diff.trim() !== "") {
    runCmd('git commit -m "fix(git): add *.xlsx to .gitignore"');
    runCmd(`git push --force-with-lease origin HEAD:${branch}`);
    console.log(`  ✅ Synced .gitignore on ${branch}`);
  } else {
    console.log(`  ✅ ${branch} .gitignore already up to date`);
  }
}

runCmd("git checkout -f DS-064");
console.log("Done syncing .gitignore repo-wide!");
